...
- (void)testLoginPhone
{
    ...
    [SwedbankTestableLogin setupForTests];
    SwedbankTestableLogin *login = [SwedbankTestableLogin swedbankLoginWithUsername:SWEDBANK_USERNAME andPassword:SWEDBANK_PASSWORD];
    ...
    if (status == 2) {
        if (login.loginStep == 0) {
            SwedbankLoginParser *loginParser = [login performSelector:@selector(loginParser)];
            ...
            STAssertNotNil(loginParser.nextLoginStepUrl, @"a next step url should have been parsed");        
        }
        else if (login.loginStep == 1) {
            SwedbankLoginParser *loginParser = [login performSelector:@selector(loginParser)];
            STAssertNotNil(loginParser.passwordField, @"There should be a password field");
        }
        
        STFail(@"Authentication failed for Swedbank with message: %@", loginErrorMessage);
    }
}
